...
var DrawHelper = (function() {
    ...
    var host = NPMAP3D.NPUtil.getHost() + 'lib/Cesium/Assets';
    ...
    function convertPosition(options) {
        if (options.constructor.name == 'Array') {
            ...
            var p = NPMAP3D.NPUtil.CoordinateHelper.coordinateFromBasemap(Cesium.Math.toDegrees(l.longitude), Cesium.Math.toDegrees(l.latitude));
    ...
    _.prototype.measureDistance = function(option) {
        ...
        this.startDrawingPolyline({
            callback: function(positions) {
                ...
                var length = 0;
                for (var i = points.length - 1; i >= 0; i--) {
                    length += NPMAP3D.NPUtil.distinct(points[i], points[i - 1]);
                    if (i == 1) {
                        break;
                    }
                }
    ...
    _.prototype.measureArea = function(option) {
        ...
        this.startDrawingPolygon({
            callback: function(positions) {
                ...
                var lastPosition = positions[positions.length - 1];
                var points = convertPosition(positions);
                var text = NPMAP3D.NPUtil.getArea(points).toFixed(2);
                if (text > 100000) {
